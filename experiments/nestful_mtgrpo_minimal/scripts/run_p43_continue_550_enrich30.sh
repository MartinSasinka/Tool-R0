#!/usr/bin/env bash
# Continue P43 PROFILE_1000 + 30% enrichment: checkpoint@350 → target 550.
#
# From repo root (Linux / RunPod):
#   bash experiments/nestful_mtgrpo_minimal/scripts/run_p43_continue_550_enrich30.sh
#
# Defaults:
#   SRC_RUN = .../continue350_enrich30
#   CKPT    = .../checkpoints/adapter_epoch_14  (global_step=350)
#
# Speed leftovers:
...
CFG="$MINIMAL/configs/qwen3_p43_profile1000_dynamic_online_continue550_enrich30.yaml"

SRC_RUN="${SRC_RUN:-$MINIMAL/outputs/qwen3_p43_profile1000_dynamic_online_continue350_enrich30}"
CKPT="${CKPT:-$SRC_RUN/checkpoints/adapter_epoch_14}"
...
if [[ ! -f "$CFG" ]]; then
  echo "[p43-continue550] ERROR: missing config $CFG" >&2
  exit 1
fi
if [[ ! -d "$ADAPTER/lib" ]]; then
  echo "[p43-continue550] ERROR: missing adapter at $ADAPTER" >&2
  exit 1
fi
if [[ ! -f "$PROFILE" ]]; then
  echo "[p43-continue550] ERROR: missing profile dataset $PROFILE" >&2
  exit 1
fi
if [[ ! -f "$ENRICH" ]]; then
  echo "[p43-continue550] ERROR: missing enrichment dataset $ENRICH" >&2
  exit 1
fi
if [[ ! -f "$CKPT/adapter_config.json" ]]; then
  echo "[p43-continue550] ERROR: resume checkpoint invalid: $CKPT" >&2
  exit 1
fi
if [[ ! -f "$CKPT/sampler_state.json" ]]; then
  echo "[p43-continue550] ERROR: missing sampler_state.json in $CKPT" >&2
  exit 1
fi
if [[ ! -f "$CKPT/trainer_state.json" ]]; then
  echo "[p43-continue550] ERROR: missing trainer_state.json in $CKPT" >&2
  exit 1
fi
if [[ ! -f "$CKPT/optimizer.pt" ]]; then
  echo "[p43-continue550] ERROR: missing optimizer.pt in $CKPT" >&2
  exit 1
fi

echo "[p43-continue550] SRC_RUN=$SRC_RUN"
echo "[p43-continue550] CKPT=$CKPT"
echo "[p43-continue550] CFG=$CFG"
echo "[p43-continue550] SYNTHETIC_TOOLS_DIR=$SYNTHETIC_TOOLS_DIR"
echo "[p43-continue550] CANARY_TRAJ_LOG=$CANARY_TRAJ_LOG (expect 0)"
echo "[p43-continue550] mix: profile=0.70 enrichment=0.30 → target 550"
echo "[p43-continue550] vllm_enforce_eager: NOT overridden (yaml false)"
...
print(f"[p43-continue550] precheck trainer_state.global_step={ts.get('global_step')}")
print(f"[p43-continue550] precheck sampler bootstrap_complete={ss.get('bootstrap_complete')} "
      f"n_observed={ss.get('n_observed')} restored_mode={ss.get('sampler_mode')}")
assert int(ts.get("global_step") or -1) == 350, ts.get("global_step")
...
print(f"[p43-continue550] precheck pools profile={n_prof} enrichment={n_enr}")
assert n_prof == 1000 and n_enr == 500, (n_prof, n_enr)
print("[p43-continue550] precheck OK")
PY
