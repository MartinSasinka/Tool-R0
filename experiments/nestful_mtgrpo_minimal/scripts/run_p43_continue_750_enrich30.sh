#!/usr/bin/env bash
# Continue P43 PROFILE_1000 + 30% enrichment: checkpoint@550 → target 750.
#
# From repo root (Linux / RunPod):
#   bash experiments/nestful_mtgrpo_minimal/scripts/run_p43_continue_750_enrich30.sh
#
# Defaults:
#   SRC_RUN = .../continue550_enrich30
#   CKPT    = .../checkpoints/adapter_epoch_19  (global_step=550)
set -euo pipefail
...
CFG="$MINIMAL/configs/qwen3_p43_profile1000_dynamic_online_continue750_enrich30.yaml"

SRC_RUN="${SRC_RUN:-$MINIMAL/outputs/qwen3_p43_profile1000_dynamic_online_continue550_enrich30}"
CKPT="${CKPT:-$SRC_RUN/checkpoints/adapter_epoch_19}"
...
if [[ ! -f "$CFG" ]]; then
  echo "[p43-continue750] ERROR: missing config $CFG" >&2
  exit 1
fi
if [[ ! -d "$ADAPTER/lib" ]]; then
  echo "[p43-continue750] ERROR: missing adapter at $ADAPTER" >&2
  exit 1
fi
if [[ ! -f "$PROFILE" || ! -f "$ENRICH" ]]; then
  echo "[p43-continue750] ERROR: missing profile/enrichment jsonl" >&2
  exit 1
fi
if [[ ! -f "$CKPT/adapter_config.json" || ! -f "$CKPT/optimizer.pt" \
   || ! -f "$CKPT/sampler_state.json" || ! -f "$CKPT/trainer_state.json" ]]; then
  echo "[p43-continue750] ERROR: resume checkpoint invalid: $CKPT" >&2
  exit 1
fi

echo "[p43-continue750] SRC_RUN=$SRC_RUN"
echo "[p43-continue750] CKPT=$CKPT"
echo "[p43-continue750] CFG=$CFG"
echo "[p43-continue750] mix: profile=0.70 enrichment=0.30 → target 750"
...
print(f"[p43-continue750] precheck trainer_state.global_step={ts.get('global_step')}")
print(f"[p43-continue750] precheck sampler bootstrap_complete={ss.get('bootstrap_complete')} "
      f"n_observed={ss.get('n_observed')} restored_mode={ss.get('sampler_mode')}")
assert int(ts.get("global_step") or -1) == 550, ts.get("global_step")
...
print(f"[p43-continue750] precheck pools profile={n_prof} enrichment={n_enr}")
assert n_prof == 1000 and n_enr == 500, (n_prof, n_enr)
print("[p43-continue750] precheck OK")
PY
...
python run.py --mode train \
  ...
  --override interaction.tool_call_slack=1 \
  "$@"
