"""Regression tests for merge_agentic_workers.py (multi-GPU worker merge).
...
Covers:
  ...
  4. sample_id is renumbered sequentially agentic_v4_<short>_NNNNNN with no
...
Run:  python experiments/nestful_synthetic_curriculum_v3/tests/test_merge_agentic_workers.py
"""
...
from merge_agentic_workers import merge_stage  # noqa: E402
...
try:
    ...
    rows0 = [
        _row("agentic_v4_stage2_000001", "What is the total of A and B?", CALLS_A),
        _row("agentic_v4_stage2_000002", "How much fuel is left after the trip?",
            CALLS_B),
    ]
    ...
    rows1 = [
        _row("agentic_v4_stage2_000001", "What is the total of A and B?", CALLS_C),
        _row("agentic_v4_stage2_000002",
            "How much fuel remains once the trip finishes?", CALLS_B),
        _row("agentic_v4_stage2_000003", "What is the average speed?", CALLS_C),
    ]
    ...
    short_ids_ok = all(sid.startswith("agentic_v4_stage2_") for sid in ids)
    check("renumbered ids all use agentic_v4_stage2_ prefix", short_ids_ok, ids)
    check("renumbered ids are sequential 000001..N with no collisions",
         ids == [f"agentic_v4_stage2_{i + 1:06d}" for i in range(len(merged))],
         ids)
