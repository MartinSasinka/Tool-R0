...
echo "[launch] rollout        = T=$ROLLOUT_TEMPERATURE_VAL N=$ROLLOUT_N_VAL achievable_win=$ROLLOUT_REQUIRE_ACHIEVABLE_WIN_VAL"
echo "[launch] weak_solver    = $WEAK_SOLVER_BACKEND_VAL $LOCAL_WEAK_MODEL_VAL (4bit=$LOCAL_WEAK_4BIT_VAL) weak_T=$SOLVER_MT_WEAK_TEMPERATURE_VAL"
echo "[launch] diversity      = enforce_after=$DIVERSITY_ENFORCE_AFTER_VAL weak_score_cap=$DIVERSITY_MAX_SAME_WEAK_SCORE_VAL failure_cap=$DIVERSITY_MAX_SAME_FAILURE_TYPE_VAL"
echo "[launch] early-stop     = MIN_ACCEPT_RATE=$MIN_ACCEPT_RATE_VAL (0=disabled) WARMUP=$WARMUP_BATCHES_VAL"
echo "======================================================================="
