#!/usr/bin/env python3
"""Merge per-GPU-worker agentic dataset shards into one deduped, renumbered
canonical dataset.

Each worker launched by ``launch_multi_gpu_workers.sh`` runs the FULL
orchestrator pipeline independently against its own ``--output-dir``, so
in-corpus dedup / diversity caps are only enforced WITHIN a single worker.
This script closes that gap AFTER all workers finish:

  1. loads ``filtered/<stage_file>.jsonl`` from every worker directory, in a
     deterministic worker order (sorted path), so re-running the merge is
     reproducible;
  2. re-applies CROSS-worker dedup by ``(question_hash, trace_hash)`` — the
     exact same hashes ``DedupIndex`` uses inside a single run (see
     ``lib/agentic_data/quality.py`` and
     ``lib/nestful_like_generator.question_hash`` / ``trace_hash``) — keeping
     the FIRST worker's copy of any duplicate and dropping the rest;
  3. renumbers ``sample_id`` sequentially per stage
     (``agentic_v4_<short_stage>_NNNNNN``), so IDs never collide even though
     every worker independently started counting from 1;
  4. writes the merged stage file(s) to ``--output-dir/filtered/``, plus a
     ``MERGE_REPORT.md`` / ``MERGE_REPORT.json`` with per-worker/per-stage
     counts, dropped cross-worker duplicates, and merged corpus_stats
     (``lib/agentic_data/distribution.py``) diversity numbers.

This script does NOT re-run any solver/judge/execution gates — every row it
merges was ALREADY accepted by its worker's full orchestrator pipeline
(hard-trace validation, semantic compatibility, solver-gap, GRPO-signal
rollout probe, diversity caps, LLM judge). It only removes cross-worker
duplicates and renumbers IDs.
...
Usage (repo root):
    ...
    # or list workers explicitly:
    python merge_agentic_workers.py --workers .../gpu0 .../gpu1 .../gpu2 .../gpu3 \\
        --output-dir .../curriculum_v4_nestful_like_agentic_openrouter
"""
...
def merge_stage(stage: str, worker_dirs: List[str], *, max_rows: int = None
    ...
    for i, row in enumerate(merged):
        row["sample_id"] = f"agentic_v4_{short}_{i + 1:06d}"
...
def write_merge_report(out_root: str, per_stage_reports: List[Dict[str, Any]],
    ...
    with open(os.path.join(reports_dir, "MERGE_REPORT.json"), "w",
             encoding="utf-8") as fh:
        json.dump({"per_stage": per_stage_reports,
                   "corpus_stats_by_stage": stats_by_stage}, fh, indent=2,
                  ensure_ascii=False)

    lines = ["# MERGE_REPORT — multi-GPU worker merge", ""]
    ...
    with open(os.path.join(reports_dir, "MERGE_REPORT.md"), "w",
             encoding="utf-8") as fh:
        fh.write("\n".join(lines) + "\n")
...
def main() -> int:
    ...
    ap.add_argument("--workers-glob", default=None,
                    help="glob pattern matching worker output dirs "
                         "(e.g. '.../agentic_workers/gpu*')")
    ap.add_argument("--output-dir", required=True,
                    help="canonical output dir; filtered/*.jsonl + "
                         "reports/MERGE_REPORT.* are written here")
    ...
    worker_dirs = sorted({os.path.abspath(w) for w in worker_dirs
                          if os.path.isdir(w)})
    if not worker_dirs:
        print("[merge] ERROR: no worker directories found "
              f"(--workers={args.workers} --workers-glob={args.workers_glob})",
              file=sys.stderr)
        return 2

    print(f"[merge] merging {len(worker_dirs)} worker(s):")
    ...
    for stage in args.stages:
        ...
        print(f"[merge] {stage}: {report['total_loaded']} loaded -> "
    ...
    print(f"[merge] wrote reports/MERGE_REPORT.md and .json under {out_root}")
    return 0
